  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...
        'en-US': alternates.en ? `${SITE_URL}${alternates.en}` : canonical,
        'uz-UZ': alternates.uz ? `${SITE_URL}${alternates.uz}` : `${SITE_URL}/uz/blog/${post.slug}`,
        'x-default': canonical,
      },
    },
    robots: {
...
export default async function ArticlePage({ params }: { params: Promise<Params> }) {
  ...
  const related = getRelatedPosts('en', slug, 5);
  return (
    <main className="container mx-auto px-4 py-8 prose max-w-3xl">
      <ArticleJsonLd
        mainEntityOfPage={`/en/blog/${post.slug}`}
      ...
      <p className="text-gray-500 text-sm">{new Date(post.frontmatter.date).toLocaleDateString('en-US')} • {post.readingTimeMinutes} min</p>
      {post.toc.length > 0 && (
      ...
      <section>
        ...
        <ul className="list-disc pl-5">
          {related.slice(0, 5).map(r => (
            <li key={r.slug}><Link href={`/en/blog/${r.slug}`}>{r.frontmatter.title}</Link></li>
          ))}
        </ul>
      </section>
    </main>
