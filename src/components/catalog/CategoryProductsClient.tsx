...
export default function CategoryProductsClient({
  ...
  const categoryTitle = useMemo(() => {
    // Всегда используем categoryName из родителя (из БД), если передано
    if (categoryName && categoryName.trim()) {
      return categoryName;
    }

    // Fallback: только если categoryName не передано (крайний случай)
        ...
        <section className="container-section section-y mt-0 pt-4 sm:pt-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
              <div className="relative flex-grow max-w-[420px]">
                <input
                  ...
                  className="min-h-11 w-full px-4 py-2.5 !text-[#660000] rounded-xl border border-[#660000] bg-white hover:bg-[#fafafa] focus:outline-none focus:ring-2 focus:ring-[#660000] focus:ring-opacity-50 transition-all duration-200"
                />
                {searchQuery && (
                  <button
                    type="button"
              ...
              <button onClick={() => setDrawerOpen(true)} className="md:hidden btn-ghost min-h-11 px-3 py-2" aria-expanded={drawerOpen} aria-controls="mobile-filters">
          ...
          <div className="flex gap-8">
            ...
            <div className="min-w-0 flex-1">
              ...
              {bootLoading ? (
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 items-stretch gap-3 sm:gap-4 md:gap-6">
